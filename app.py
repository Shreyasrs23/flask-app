...
with open(r"C:\Users\Shreyas\Downloads\Risk_analysis_model.pkl", 'rb') as f:
    model=pickle.load(f)
    ...
    try:
        ...
        if query_result:
            ...
            columns_to_drop = ['firstName', 'email', 'PhoneNumber', 'PassportNumber', 'PhysicalAddress', 'SevisID',
                               'I-94DocNumber', 'ApartmentNo', 'lastname','_id']
            ...
            data_cleaned['daysFromPortofEntry'] = pd.to_datetime(data_cleaned['daysFromPortofEntry'], format='%m/%d/%Y', errors='coerce')
            today = pd.Timestamp(date.today())
            data_cleaned['daysFromPortofEntry'] = (data_cleaned['daysFromPortofEntry']- today).dt.days
